    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const subCategoriesResponse = await axios.get(`https://weckwerthblog.wordpress.com/wp-json/wp/v2/categories?parent=${bydgoszczCategoryId}&per_page=100`);

                const categoriesData = [];

                for (const category of subCategoriesResponse.data) {
                    const postsResponse = await axios.get(`https://weckwerthblog.wordpress.com/wp-json/wp/v2/posts?categories=${category.id}&per_page=1&_embed`);
