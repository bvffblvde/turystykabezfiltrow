    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const latestWycieczkiResponse = await axios.get(`https://weckwerthblog.wordpress.com/wp-json/wp/v2/posts?categories=${wycieczkiCategory.id}&per_page=${projectLimit}`);

                // Если есть seasonCard, добавляем последний пост с тегом "sezon"
                if (seasonCard) {
                    const tagsResponse = await axios.get('https://weckwerthblog.wordpress.com/wp-json/wp/v2/tags?per_page=100');
                    ...
                    const latestSezonResponse = await axios.get(`https://weckwerthblog.wordpress.com/wp-json/wp/v2/posts?tags=${sezonTag.id}&per_page=1&_embed`);
                    ...
                    const mediaPromises = mediaIds.map((mediaId) =>
                        axios.get(`https://weckwerthblog.wordpress.com/wp-json/wp/v2/media/${mediaId}`)
                    );
